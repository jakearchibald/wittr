...
export default {
  ...
  'sw-waiting': function() {
    return remoteEval(function() {
      return navigator.serviceWorker.getRegistration('/').then(reg => {
        if (!reg) return 'noreg';
        if (!reg.waiting) return 'nowaiting';
        return 'waiting';
      });
    }).then(result => {
      if (result == 'noreg') return ["Doesn't look like there's a service worker registered at all!", 'sad.gif', false];
      if (result == 'nowaiting') return ["There's no service worker waiting", 'nope.gif', false];
  ...
  'sw-active': function() {
    return remoteEval(function() {
      ...
      if (result == 'noreg') return ["Doesn't look like there's a service worker registered at all!", 'sad.gif', false];
      if (result == 'waiting') return ["There's still a service worker waiting", 'nope.gif', false];
